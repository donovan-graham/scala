...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  //  def union(that: TweetSet): TweetSet = new NonEmpty(elem, left union that, right union that)

  def mostRetweeted: Tweet = {
    def maxOf(a: Tweet, b: Tweet): Tweet =
      if (a.retweets > b.retweets) a
      else b

    if (left.isInstanceOf[Empty] && right.isInstanceOf[Empty]) elem
    else if (right.isInstanceOf[Empty]) maxOf(elem, left.mostRetweeted)
    else if (left.isInstanceOf[Empty]) maxOf(elem, right.mostRetweeted)
    else maxOf(elem, maxOf(left.mostRetweeted, right.mostRetweeted))
  }
